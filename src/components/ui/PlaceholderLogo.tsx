...
export const PlaceholderLogo: React.FC<{ name: string; className?: string }> = ({ name, className = '' }) => (
  <div
    title={name}
    className={`flex flex-col items-center justify-center gap-1 rounded-lg border border-brand/25 bg-brand/[0.06] px-3 ${className}`}
  >
    <span className="text-brand text-sm font-black tracking-wide">{initials(name)}</span>
